...
RUN apk --update --no-cache add bash multirun nginx libzip-dev zip jq g++ make rabbitmq-c-dev libpng-dev oniguruma-dev autoconf supervisor curl-dev libxml2-dev icu-dev && \
    docker-php-ext-configure bcmath --enable-bcmath && \
    docker-php-ext-install pdo_mysql bcmath gd zip pcntl curl soap intl && \
